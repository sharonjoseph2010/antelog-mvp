-- Add forwarding chain to requests table
ALTER TABLE requests ADD COLUMN forwarding_chain JSONB DEFAULT '[]'::jsonb;

-- Create request_forwards tracking table
...
USING (
  ...
  -- User can see if request was forwarded to them via network/group
  EXISTS (
...
BEGIN
  ...
  SELECT en.profile_id INTO mutual_friend
  FROM get_extended_network(user_a) en
  WHERE en.profile_id = user_b
  LIMIT 1;
  
  IF mutual_friend IS NOT NULL THEN
    -- Find the connecting friend
    SELECT ARRAY[
      (SELECT full_name FROM profiles WHERE id = user_a),
      mutual_friends[1],
      (SELECT full_name FROM profiles WHERE id = user_b)

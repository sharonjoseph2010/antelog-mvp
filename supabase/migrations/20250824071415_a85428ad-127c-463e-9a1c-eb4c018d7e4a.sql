...
-- 3. Additional security function for logging admin access (for UPDATE/DELETE only)
...
AS $$
BEGIN
  -- Log admin access to other users' profiles for sensitive operations
...
-- Create trigger for logging admin access (UPDATE/DELETE only)
DROP TRIGGER IF EXISTS log_admin_profile_access_trigger ON public.profiles;

...
-- First, drop existing sync functions that may interfere
DROP TRIGGER IF EXISTS sync_directory_entries_trigger ON list_items;
DROP TRIGGER IF EXISTS sync_directory_list_changes_trigger ON lists;
DROP FUNCTION IF EXISTS public.sync_directory_entries();
DROP FUNCTION IF EXISTS public.sync_directory_on_list_change();
